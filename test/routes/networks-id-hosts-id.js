...
lab.experiment('/networks/:networkIp/hosts/:hostIp', function () {
  ...
  lab.experiment('DELETE', function () {
    lab.test('allocate and delete host', function (done) {
        ...
        function (err, res) {
          ...
          supertest(app).post('/networks/10.255.252.0/hosts').expect(200).end(
            function (err, res) {
              ...
              supertest(app).delete('/networks/10.255.252.0/hosts/10.255.252.1').expect(200, done);
          });
        });
    });
    lab.test('delete non allocated host', function (done) {
      supertest(app).post('/networks').expect(200).end(
        function (err, res) {
          if (err) { return done(err); }
          Lab.expect(res.body.networkIp).to.equal('10.255.252.0');
          supertest(app).delete('/networks/10.255.252.0/hosts/10.255.252.1').expect(404, done);
        });
    });
    lab.test('invalid host asdf', function (done) {
      supertest(app).post('/networks').expect(200).end(
        function (err, res) {
          if (err) { return done(err); }
          Lab.expect(res.body.networkIp).to.equal('10.255.252.0');
          supertest(app).delete('/networks/10.255.252.0/hosts/asdf').expect(400, done);
        });
    });
    lab.test('invalid host 10.0.0.a', function (done) {
      supertest(app).post('/networks').expect(200).end(
        function (err, res) {
          if (err) { return done(err); }
          Lab.expect(res.body.networkIp).to.equal('10.255.252.0');
