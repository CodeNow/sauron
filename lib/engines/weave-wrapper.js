...
function launch(options, cb) {
  ...
  var cmd = 'weave launch -password ' + options.password;
...
function detach(options, cb) {
  ...
  runCmd(cmd, handleErrNoReport(cb, 'detach failed', { cmd: cmd }));
}
...
/**
 * Handles weave errors but does not report them to rollbar.
 * @see handleErr
 */
function handleErrNoReport (cb, message, errDebug) {
  return handleErr(function (err) {
...
/**
 * Handles errors with weave command methods.
 * @param cb Callback to execute for the weave method.
 * @param message Message associated with the weave command.
 * @param errDebug Information about the command.
 */
