...
Events.handleStart = function (data, cb) {
  ...
  Docker.findDocksByOrgId(data.orgId, function (err, peers) {
    ...
    const parsedHost = url.parse(data.dockerUri)
    const dockerHost = parsedHost.host
    const dockerHostname = parsedHost.hostname

    const hostnames = peers.map(function (peer) {
      return url.parse('http://' + peer.Host).hostname
    })

    // ensure this target exist in peers, error if not
    const index = hostnames.indexOf(dockerHostname)
    if (index === -1) {
...
Events.handleDockerEventStreamDisconnected = function (data, cb) {
  ...
  logger.info('call')
  const parsedHost = url.parse(data.host)
  const parsedHostname = parsedHost.hostname
  const dockerHost = parsedHost.host
  // gets all peers
  Docker.doesDockExist(dockerHost, data.org).asCallback(function (err, doesExist) {
    if (err) {
...
Events.handleStarted = function (data, cb) {
  ...
  const dockerHost = url.parse(data.host).host
  const githubOrgId = data.org
  // gets all peers
  Docker.doesDockExist(dockerHost, githubOrgId).asCallback(function (err, doesExist) {
    if (err) {
      return cb(new WorkerError('handleStarted: unknown error', err))
    }
    if (!doesExist) {
      logger.trace({ dockUri: data.host }, 'dock does not exist')
      return cb(new WorkerStopError('dock no longer exists', {}, {
        level: 'trace'
      }))
    }

    WeaveWrapper.attach(data.id, dockerHost, githubOrgId, function (err, containerIp) {
      if (err) {
      ...
      data.githubOrgId = parseInt(githubOrgId, 10)
...
Events._isNetworkNeeded = function (data) {
  const blackList = process.env.NETWORK_BLACKLIST.split(',')
  const isBlacklisted = blackList.some(function (item) {
