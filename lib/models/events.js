...
Events._removeWeavePeer = function (hostname, orgId, cb) {
  ...
  log.info(logData, 'Events._removeWeavePeer')
  Docker.findLightestOrgDock(orgId, function (err, dock) {
    ...
    log.info(put({ dock: dock }, logData), '_removeWeavePeer found dock')
    var dockerHostLabel = dock.Labels.find(function (label) {
      return label.name === 'dockerHost'
    })
    if (!dockerHostLabel || !dockerHostLabel.value) {
      return cb(new TaskFatalError('dock-removed', 'Dock has not host data'))
    }
    log.info(put({ dockerHostLabel: dockerHostLabel }, logData), '_removeWeavePeer found dock host')
    var dockerHost = dockerHostLabel.value
...
/**
 * Forget weave peer.
 * 1. find all host which should foget about weave peer
...
Events._forgetWeavePeer = function (hostname, orgId, cb) {
  ...
  Peers.getList(orgId, function (err, peers) {
    if (err) { return cb(err) }
    peers.forEach(function (peer) {
      var peerHost = url.parse(peer.dockerUri).host
...
Events.handleDockRemoved = function (data, cb) {
  ...
  async.parallel([
    Events._removeWeavePeer.bind(Events, parsedHostname, data.githubId),
    Events._forgetWeavePeer.bind(Events, parsedHostname, data.githubId)
  ], cb)
};
