...
Events.handleStart = function (data, cb) {
  ...
  Swarm.findDocksByOrgId(data.orgId, function (err, peers) {
...
Events._removeWeavePeer = function (hostname, orgId, cb) {
  ...
  Swarm.findLightestOrgDock(orgId, function (err, dock) {
    if (err) {
      return cb(err)
    }
    if (!dock) {
...
Events._forgetWeavePeer = function (hostname, orgId, cb) {
  ...
  Swarm.findDocksByOrgId(orgId, function (err, orgDocks) {
    if (err) { return cb(err) }
 ...
 * @returns  {Promise} (err)
 */
Events.handleSwarmEventStreamDisconnected = function (data, cb) {
  var logger = log.child({
    method: 'handleSwarmEventStreamDisconnected',
    data: data
  })
  logger.info('call')
  ...
  // gets all peers
  Swarm.doesDockExist(dockerHost).asCallback(function (err, doesExist) {
    if (err) {
      logger.error({ err: err }, 'does docker exist error')
      return cb(new WorkerError('handleSwarmEventStreamDisconnected: unknown error', err))
    }
    if (!doesExist) {
