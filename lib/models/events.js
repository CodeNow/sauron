  ...
  Peers.doesDockExist(data.host, function (err, doesExist) {
    ...
    WeaveWrapper.attach(data.id, dockerHost, orgId, function (err, containerIp) {
      if (err) {
        ...
        // 3 types of error can happen:
        // 1) container is not running - container exited
        // 2) container died - container exited
        // 3) no such container - can happen only with runnable container.
        // we dont' expect it to happen to the user container
        // if such error occur we don't need to do anything: no need to publish `container.network.attach-failed`
        if (statusCode === 409) {
