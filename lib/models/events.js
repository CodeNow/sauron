...
Events.handleStarted = function (data, cb) {
  var logger = log.child({ data: data })
  logger.info('handleStarted')
  ...
  Docker.doesDockExist(dockerHost, function (err, doesExist) {
    ...
    if (!doesExist) {
      logger.trace({ dockUri: data.host }, 'handleStarted: dock does not exist');
    ...
    WeaveWrapper.attach(data.id, dockerHost, orgId, function (err, containerIp) {
      if (err) {
        logger.error({ err: err }, 'handleStarted attach error');
...
Events._isWeaveContainer = function (data) {
  return !!~data.from.indexOf(process.env.WEAVE_IMAGE_NAME);
};
