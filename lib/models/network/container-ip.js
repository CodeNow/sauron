 ...
 */
function setContainerIp(containerId, ipaddr, force, cb) {
  ...
  redis.hgetall(key, function(err, containersMapped) {
    ...
    if (ip && ip === ipaddr) {
      return cb();
    } else if (ip && ip !== ipaddr) {
      errData.oldContainerId = containersMapped[ip];
      errData.oldIp = ip;
      return cb(error.boom(409, 'container is mapped to a different IP', errData));
    }

    // if we are forcing, do not check current mapping
    if (force) {
      return redis.hset(key, ipaddr, containerId, cb);
    }
 ...
 * @return function (err)
 */
function removeContainerIp(containerId, ipaddr, force, cb) {
  var key = process.env.WEAVE_NETWORKS+':container';
  var errData = {
    ip: ipaddr,
  ...
  redis.hget(key, ipaddr, function(err, containerMapped) {
    ...
    if (!containerMapped) {
      return cb(error.boom(409, 'container is not mapped to an ip', errData));
    }
    if (containerMapped !== containerId && !force) {
      return cb(error.boom(409, 'ip is mapped to a different container', errData));
    }
    redis.hdel(key, ipaddr, cb);
  });
}
