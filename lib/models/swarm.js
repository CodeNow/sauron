 ...
 */
Swarm._findDocksByOrgId = function (nodes, orgId) {
 ...
 */
Swarm.findDocksByOrgId = function (orgId, cb) {
  const logger = log.child({
    orgId: orgId,
    method: 'findDocksByOrgId'
  })
  logger.info('call')
  Swarm.info().asCallback(function (err, docks) {
    if (err) { return cb(err) }
    const orgDocks = Swarm._findDocksByOrgId(docks, orgId)
 ...
 */
Swarm.findLightestOrgDock = function (orgId, cb) {
  const logger = log.child({
    orgId: orgId,
    method: 'findLightestOrgDock'
  })
  logger.info('call')
  Swarm.findDocksByOrgId(orgId, function (err, docks) {
    if (err) { return cb(err) }
 ...
 */
Swarm.doesDockExist = function (dockerHost) {
  const logger = log.child({
