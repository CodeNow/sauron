 ...
 */
WeaveWrapper.launch = function (options, cb) {
  if (!options ||
    typeof options.password !== 'string' ||
    typeof options.peers !== 'object') {
    return cb(error.createAndReport(400, 'invalid input', options));
  }

  var cmd = [process.env.WEAVE_PATH, 'launch-router', '--no-dns',
...
WeaveWrapper.attach = function (containerId, cb) {
  ...
  WeaveWrapper._runCmd(cmd,
    WeaveWrapper.handleErr(cb, 'attach failed', { cmd: cmd }));
};
