...
WeaveWrapper.attach = function (containerId, dockerHost, githubId, cb) {
  var log = log.child({ containerId: containerId, dockerHost: dockerHost, githubId: githubId })
  log.info('WeaveWrapper.attach')
  if (!isString(containerId)) {
    log.error('attach invalid containerId');
  ...
  function handleAttach (err, stdout) {
    ...
    if(!ip.isV4Format(stdout)) {
      log.error({ err: err }, 'attach failed')
    ...
    log.info({ stdout: stdout }, 'attach success')
    cb(null, stdout)
  }
};
...
WeaveWrapper._handleCmdResult = function (cb, message, errDebug) {
  return function (err, stdout) {
    ...
    if (err && !_errorIs.weaveAlreadyRunning(err)) {
      ...
      var cmdError = error.createAndReport(statusCode, message, errDebug)
