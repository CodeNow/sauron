...
const FailedAttach = require('../errors/failed-attach')
const InvalidArgument = require('../errors/invalid-argument')
const WeaveError = require('../errors/weave-error')
 ...
 * @param  {String}   githubOrgId github id from the tag data
 * @param  {Function} cb          (err, allocated)
 */
WeaveWrapper.launch = function (peers, dockerHost, githubOrgId, cb) {
 ...
 */
WeaveWrapper.attach = function (containerId, dockerHost, githubOrgId, cb) {
  var logger = log.child({ containerId: containerId, dockerHost: dockerHost, githubOrgId: githubOrgId })
...
WeaveWrapper._handleCmdResult = function (cb, message, command, errDebug) {
  return function (err, stdout) {
    ...
    if (weaveError.outOfMemory()) {
      ...
      RabbitMQ.publishDockLost({
        host: DockerUtils.toDockerUrl(errDebug.host),
        githubOrgId: errDebug.githubOrgId
      })
    }
