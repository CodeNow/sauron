...
WeaveWrapper.attach = function (containerId, dockerHost, githubId, cb) {
  var logData = { containerId: containerId, dockerHost: dockerHost, githubId: githubId }
  log.info(logData, 'WeaveWrapper.attach');
  if (!isString(containerId)) {
    log.error(logData, 'attach invalid containerId');
  ...
  function handleAttach (err, stdout) {
    ...
    if(!ip.isV4Format(stdout)) {
      log.info(put({ err: err }, logData), 'attach failed');
    ...
    log.info(put({ stdout: stdout }, logData), 'attach success');
    cb(null, stdout);
  }
};
