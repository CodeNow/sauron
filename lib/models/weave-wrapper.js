...
/**
 * loads certs required for docker
 * @throws {Error} If certs not loaded
 */
WeaveWrapper.setCerts = function () {
  var certPath = process.env.DOCKER_CERT_PATH;
  this.certs = {};
  this.certs.ca = fs.readFileSync(join(certPath, '/ca.pem'));
  this.certs.cert = fs.readFileSync(join(certPath, '/cert.pem'));
  this.certs.key = fs.readFileSync(join(certPath, '/key.pem'));
};

/**
 * run cmd in a child process
 * @param  {String}   cmd  command to run
 * @param  {Object}   envs environment variables to set
 * @param  {Function} cb   (err, stdout)
 */
WeaveWrapper._runCmd = function (cmd, envs, cb) {
...
WeaveWrapper.launch = function (peers, dockerHost, cb) {
  ...
  var envs = put(this.certs, {
    DOCKER_HOST: 'tcp://' + dockerHost
  });

  WeaveWrapper._runCmd(cmd, envs,
 ...
 * @param  {Function} cb (err, stdout)
 */
WeaveWrapper.attach = function (containerId, dockerHost, cb) {
...
var _errorIs = {
  ...
  noSuchContainer: function (err) {
    return /No such container/.test(err.message);
  },
