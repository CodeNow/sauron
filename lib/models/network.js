  ...
  weave.status(function(err, status) {
    ...
    networkFactory.getRouters(function(err, peers) {
      ...
      networkFactory.checkMapping(ip.address(), function(err, routerIpAddr) {
        ...
        if (!routerIpAddr) {
          return networkFactory.createHost(process.env.WEAVE_ROUTER_NETWORK,
...
function launchWeave(routerIpAddr, peers, cb) {
  ...
  weave.launch(options, function (err) {
    if (err) {
      networkFactory.removeHost(process.env.WEAVE_ROUTER_NETWORK, routerIpAddr);
      return cb(err);
    }
  });
}
...
function createHostAddress (networkIp, cb) {
  networkFactory.createHost(networkIp, cb);
}

function removeHostAddress (networkIp, hostIp, cb) {
  networkFactory.createHost(networkIp, hostIp, cb);
}

function createNetworkAddress (name, cb) {
  networkFactory.createNetworkAddress(name, cb);
}

function removeNetworkAddress (networkIp, cb) {
  networkFactory.createNetworkAddress(networkIp, cb);
}
