...
function getPeers (cb) {
  redis.smembers(process.env.WEAVE_NETWORKS+':'+process.env.WEAVE_PEERS, cb);
}
 ...
 * @param  {Function} cb (err, [peers])
 */
function addPeer (ip, cb) {
  redis.sadd(process.env.WEAVE_NETWORKS+':'+process.env.WEAVE_PEERS, ip, cb);
}
