'use strict';
require('loadenv')();

var ErrorCat = require('error-cat');
var error = new ErrorCat();
var request = require('request');
var isEmpty = require('101/is-empty');

var log = require('../logger.js')();

module.exports = Peers;

/**
 * Module used to keep track of weave network peers
 */
function Peers () { }

/**
 * get array of routers for a specific org
 * @param  {String}   org id of org
 * @param  {Function} cb  (err, [peers])
 *                        peers can be empty array
 */
Peers.getList = function (org, cb) {
  log.info({ org: org }, 'getList');
  request.get(process.env.MAVIS_URL + '/docks', function (err, res, body) {
    log.trace({ body: body }, 'response from mavis');
    if (err || res.statusCode !== 200) {
      log.error({ err: err, statusCode: res && res.statusCode }, 'failed to get docks from mavis');
      return cb(error.createAndReport(err ? 502 : res.statusCode, 'getting docks failed', err));
    }

    var docks = JSON.parse(body)
      .filter(function (item) {
        return ~item.tags.indexOf(org);
      })
      .map(function (item) {
        return {
          dockerUri: item.host,
          orgId: item.tags.split(',')[0]
        };
      });

    log.trace({ docks: docks }, 'returning docks');
    cb(null, docks);
  });
};

/**
 * checks to see if dock is in rotation
 * @param  {[type]}   dockUri dock to check
 * @param  {Function} cb      (err, doesDockExist)
 *                            doesDockExist: true if dock is in rotation
 */
Peers.doesDockExist = function (dockUri, cb) {
  log.info({ dockUri: dockUri }, 'doesDockExist');
  // empty string list all peers
  Peers.getList('', function (err, docks) {
    if (err) { return cb(err); }
    if (isEmpty(docks)) { return cb(null, false); }

    cb(null, docks.some(function (dock) {
      return dock.dockerUri === dockUri;
    }));
  });
};
