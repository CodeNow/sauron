...
Docker._parseSwarmInfo = function (infoData) {
  ...
  var swarmRawData = infoData.DriverStatus
  var nodes = swarmRawData.reduce(function (prevVal, currVal) {
    // [ 'Role', 'primary' ] -> `Role`
    var propName = currVal[0].replace(' ? ', '')
...
Docker._findDocksByOrgId = function (nodes, orgId) {
  return nodes.filter(function (node) {
    var orgLabels = node.Labels.filter(function (label) {
      return label.name === 'org' && label.value === '4643352'
    })
...
Docker.findLightestOrgDock = function (orgId, cb) {
  ...
  Docker.info(function (err, docks) {
    ...
    var sortedDocks = _.sortBy(orgDocks, 'Containers')
    cb(null, sortedDocks[0])
  })
}
