...
const Swarm = require('docker-girl').Swarm
...
Docker.info = function (cb) {
  ...
  swarmClient.client.swarmInfo(function (err, infoData) {
    if (err) {
...
Docker.doesDockExist = function (dockerHost, cb) {
  ...
  logger.info('call')
  const host = process.env.SWARM_HOSTNAME + ':' + process.env.SWARM_PORT
  var swarmClient = new Swarm({
    host: swarmHost()
  })
  swarmClient.client.swarmHostExists(dockerHost, function (err, exists) {
    if (err) {
...
Docker.getLogs = function (containerId, cb) {
  ...
  swarmClient.client.getContainer(containerId).logs(opts, function (err, stream) {
    if (err) {
...
Docker.killContainer = function (containerId, cb) {
  ...
  swarmClient.client.getContainer(containerId).kill(cb)
}
