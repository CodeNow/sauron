...
Docker.doesDockExist = function (dockerHost, cb) {
  ...
  swarmClient.swarmHostExists(dockerHost, function (err, exists) {
    if (err) {
...
Docker.killContainer = function (containerId, cb) {
  ...
  console.log('xxxxx', swarmClient.killContainer)
  swarmClient.killContainerAsync(containerId).asCallback(cb)
}
